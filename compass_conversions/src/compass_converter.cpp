...
void CompassConverter::configFromParams() {
  ...
  this->setKeepUTMZone(
      params->has_parameter("keep_utm_zone") ? params->get_parameter("keep_utm_zone").as_bool() : this->keepUTMZone);
...
cras::expected<double, std::string> CompassConverter::computeMagneticDeclination(
  ...
  if (this->data->magneticModels[year] == nullptr) {
    const auto modelName =
      !this->forcedMagneticModelName.empty() ?
        this->forcedMagneticModelName :
        this->data->magneticModelManager->getBestMagneticModelName(modelStamp);
...
cras::expected<std::pair<double, int>, std::string> CompassConverter::computeUTMGridConvergenceAndZone(
  ...
  try {
    ...
    GeographicLib::UTMUPS::Forward(
      fix.latitude, fix.longitude, zone, isNorthHemisphere, easting, northing, utmGridConvergence, projectionScale,
      setzone);
...
cras::expected<compass_interfaces::msg::Azimuth, std::string> CompassConverter::convertSerializedMsgEvent(
  ...
  if (topicType == rosidl_generator_traits::name<Az>()) {
    ...
    return this->convertQuaternionMsgEvent(
    ...
    return this->convertPoseMsgEvent(
    ...
    return this->convertImuMsgEvent(
